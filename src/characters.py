...
class StealthArcher:
    def __init__(self, position):
        ...
        self.position = position
        self.invisible = True
        self.invisible_startedAt = time.time()
        self.alive = True
        self.target = None
    ...
    def move(self, pos, V, type):
        ...
        if time.time() - self.invisible_startedAt >= 10:
...
class Healer:
    ...
    def move(self, near):
        ...
        r = abs(pos[0] - self.position[0])
        c = abs(pos[1] - self.position[1])
        troopshealable = findTroopsSurrounding(pos)
        if(r + c <= 7):
            near.health = near.health + 5
            for troops in troopshealable:
...
def move_stealth_archers(V, type):
    if(type == 1):
        for stealtharcher in stealth_archers:
...
def findTroopsSurrounding(pos):
    ...
    for barb in barbarians:
        if(barb.alive == False):
            continue
        if((barb.position[0] - pos[0])**2 + (barb.position[1] - pos[1])**2 == 1):
            surroundingTroops.append(barb)
    for archer in archers:
        if(archer.alive == False):
            continue
        if((archer.position[0] - pos[0])**2 + (archer.position[1] - pos[1])**2 == 1):
            surroundingTroops.append(archer)
    for sarcher in stealth_archers:
        if(sarcher.alive == False):
            continue
        if((sarcher.position[0] - pos[0])**2 + (sarcher.position[1] - pos[1])**2 == 1):
            surroundingTroops.append(sarcher)
    for dr in dragons:
        if(dr.alive == False):
            continue
        if((dr.position[0] - pos[0])**2 + (dr.position[1] - pos[1])*2 == 1):
            surroundingTroops.append(dr)
    for bal in balloons:
        if(bal.alive == False):
            continue
        if((bal.position[0] - pos[0])**2 + (bal.position[1] - pos[1])**2 == 1):
...
def findNearestTroop(pos):
    ...
    closest_dist = 100000 # Initialize closest_dist
    for barb in barbarians:
        if(barb.alive == False):
            continue
